...
//		CDC_Transmit_FS(usb_msg, strlen(usb_msg));
		...
		for (uint16_t i = 0; i < jpeg_size / UART_TXSIZE; i++) {
			...
			memcpy(uart_tx_buffer, jpeg_out + i*UART_TXSIZE, UART_TXSIZE);	// Copy jpeg vram into the TX buffer data segment
...
//			}
		...
		//	  HAL_UARTEx_ReceiveToIdle(&huart1, uart_rx_buffer, 256, &uart_rx_len, 1000);
		//	  if (uart_rx_len > 0) {
		//		  CDC_Transmit_FS(uart_rx_buffer, uart_rx_len);
		//		  uart_rx_len = 0;
		//		  memset(uart_rx_buffer, 0x00, 256);
		//	  } else {
		//		  //sprintf(usb_msg, "err\r\n");
